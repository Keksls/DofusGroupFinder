...
namespace Zaapix.Client
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private async void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            var chars = await App.ApiClient.GetCharactersAsync();
            if (chars.Count == 0)
            {
                Modal.Show(
                    "Vous n'avez pas encore enregistré de Perso. Vous devez le faire avant de continuer",
                    "Création de personnage",
                    new[] { "Oui" },
                    new Action[] {
                        () => { CharactersManagerWindow charactersManagerWindow = new CharactersManagerWindow();
                            charactersManagerWindow.Owner = this;
                            charactersManagerWindow.Show();
                        }
                    });
            }
        }
